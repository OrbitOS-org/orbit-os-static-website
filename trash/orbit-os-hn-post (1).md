...
└── Linux (Debian base)
    └── Gravity RT
        ├── Gravity RT Server (Go binary) — always running, minimal footprint
...
**Gravity RT Server** is written in Go — static binary, minimal RSS, always active. It's the process parent for every `.orb` — if an app crashes, it restarts it. If the Gravity RT Server itself falls, systemd brings it back up.
...
Write code on your laptop
  → SDK connects to the real device
  → Your code calls real device APIs — GPIO, AI, camera, sensors
  → Logs stream back to your terminal in real time
  → Iterate instantly
  → When it works: package as .orb, sign, publish
```

Your code runs on your development machine — not on the device. The SDK connects it to the real device over the network, and from your laptop you can activate GPIO pins, run AI inference, configure networking, control relays, access the camera — all through the same API that production `.orb` apps use. The device is a live API target, not a deployment destination.

No SSH required. No flashing. No waiting. Dev Mode is enabled in Settings (Android-style), which activates the SDK certificate for passwordless mTLS auth — the device warns you explicitly about the security tradeoff when you enable it.

This is what Chrome DevTools did for web development. We think embedded deserves the same.
...
The installed app icons appear in the AppHub just like apps on a home screen — the same mental model, except it runs on a Raspberry Pi and is accessible from any device on your local network.
...
**Mobile tools (connect to and control Orbit OS devices from a phone):**
...
The **Development SDK Tool** (ships with all SDKs) connects to the device API directly — install packages, remove packages, stream logs. No SSH.

The **official Android app** (sideload for now, Play Store soon) has two modes: browse the Store online, or connect directly to local devices via IP + credentials + mTLS.

Third-party developers can generate their own Kotlin or Swift SDKs from our `.proto` files and build custom mobile apps for their specific Orbit OS devices — custom dashboards, industrial control panels, smart home interfaces.
...
**Raspberry Pi 4** (Broadcom BCM2711, Debian 13, no AI hardware):
- Runtime idle: **139MB RAM** / **2.3% CPU**
...
The **Go SDK and installer** are launching soon. Community Edition targets the following hardware on Debian 13:
...
We're a team of three. We've been building this for a while. The Community Edition launches soon and we'd love early testers who aren't afraid to break things and tell us what's wrong.
