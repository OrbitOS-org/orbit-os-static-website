# Local Face Recognition on Raspberry Pi 5 and Arduino UNO Q — Two Lessons in Edge AI Efficiency

We ran the same face recognition app on two edge devices for over 26 hours. No cloud. No GPU server. No Docker. Everything local, on-device, in real time.
...
|---|---|---|---|
| Arduino UNO Q | Qualcomm QRB2210 | Cortex-A53, 4 cores @ 2016 MHz | 1.70 GB |
...
This comes down to CPU architecture:
...
The Arduino UNO Q uses the **Cortex-A53** — a compact in-order core designed for energy efficiency. It executes one instruction at a time in order, with minimal speculative logic. At 2016 MHz it hits its ceiling, but it's a thermally efficient ceiling. Less silicon doing the work, less heat produced.

For sustained AI workloads where inference runs continuously, architecture matters as much as clock speed. The A53 handles the same inference pipeline with less thermal output, which matters when you're deploying something that runs for days or weeks without interruption.

---

## What's Next

The natural next step for this project is a full **access control system** — combining what we already have with GPIO control. Face is recognized, GPIO fires, relay opens a door or gate. Everything still on-device, still offline, still under 10°C delta.

Orbit OS exposes GPIO through the same `AiService`/`CameraService` model — apps declare what hardware they need in the manifest and the runtime provides it. Adding relay control means adding `GpioService` to the permissions, wiring the relay to a GPIO pin, and writing the trigger logic. No new infrastructure, no cloud dependency, no additional deployment step.
...
The Community Edition launches next month on Raspberry Pi 3/4/5/Zero 2W and Arduino UNO Q. We'd love early testers who aren't afraid to break things and tell us what's wrong.
